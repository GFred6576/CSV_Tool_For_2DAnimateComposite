...
## *分支ByGFred
关于修改部分:
- "动画、原画"关键字改为可编辑(因为要保存关键字通过保存数据为数组写入了AE首选项,关键字应避免用特殊符号如 +-*/ )
- 增加按钮用于手动重置AE编码为GBK(此按钮是因为部分扩展开启后会修改AE编码导致部分中文脚本会出现使用bug以及配置文件丢失、错乱现象)
- 增加Apply all按钮，导入csv后选中所有序列自动尝试匹配图层名称应用打表
- 增加Edit按钮，读取csv文件后点击按钮打开文件
- ！我仅测试了CSP，因为retas现在已经很少用没有测试
